<x-layout>
<x-slot name="title">Data Peminatan</x-slot> 
<x-slot name="breadcrumb_active">Peminatan</x-slot>
<x-slot name="card_footer">@CoLearn</x-slot>

<h2 class="text-center">Data Peminatan</h2>
    <div class="d-flex justify-content-between align-items-center mb-3">
        
        <a href="{{ route('peminatan.create') }}">
        ...
        <!-- Form Search -->
        <form action="{{ route('peminatan.search') }}" method="GET" class="d-flex">
...
<table class="table table-bordered text-center align-middle">
    ...
    <tbody>
        ...
        <tr>
            <td class="fw-bold">{{ $peminatan->id }}</td>
            <td>{{ ucwords($peminatan->nama) }}</td>
            <td>{{ ucwords($peminatan->jurusan->nama) }}</td>
            <td>
            
                <form action="{{ route('peminatan.destroy', $peminatan->id) }}" method="POST" class="d-inline">
            
                    <a href="#" class="me-5 mx-2" data-bs-toggle="modal" data-bs-target="#viewModal" onclick="loadpeminatan({{ $peminatan->id }})">
                        <i class="fas fa-eye" style="color: black;"></i>
                    </a>
                    <a href="{{ route('peminatan.edit', $peminatan->id) }}" class="me-5 mx-2">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-4" id="exampleModalLabel">Detail Peminatan</h1>
...
<script>
    function loadpeminatan(id) {
    const url = `/peminatan/${id}`; // URL berdasarkan rute Anda
        ...
        function capitalizeWords(str) {
            ...
            modalBody.innerHTML = `
                <table class="table table-striped table-sm">
                    ...
                    <tr><th>Peminatan</th><td>${capitalizeWords(data.nama)}</td></tr>
                    <tr><th>Jurusan</th><td>${capitalizeWords(data.jurusan.nama)}</td></tr>
                </table>
            `;
        })
        .catch(error => {
